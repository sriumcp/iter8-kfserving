...
cp install/iter8-controller/configmaps/handlers/finish.yaml tests/scratch/finish.yaml
yq w -i tests/scratch/finish.yaml spec.template.spec.containers[0].image ${IMAGE_NAME}
yq w -i tests/scratch/finish.yaml spec.template.spec.containers[0].imagePullPolicy Never
yq w -i tests/scratch/finish.yaml spec.template.spec.containers[0].env[0].value kfserving-test
yq w -i tests/scratch/finish.yaml spec.template.spec.containers[0].env[1].value sklearn-iris-experiment-1

echo "Fixed finish handler"
cat tests/scratch/finish.yaml

echo "Launching finish handler job ... "
kubectl apply -f tests/scratch/finish.yaml -n iter8-system
